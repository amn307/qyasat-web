...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return metadataFromSeo(await getSeoPage("en", "home"), settings.brand.ogImageUrl);
}
...
export default async function EnglishHomePage() {
  ...
  const seo = await getSeoPage("en", "home");

  return (
    <>
      <CmsSeoSchema schemaJson={seo.schemaJson} />
    <main className="qyasat-home" dir="ltr">
      <QyasatSiteHeader locale="en" home={home} settings={settings} />
      ...
      <section id="blog" className="qyasat-section qyasat-blog-section">
        <div className="qyasat-section-head">
          <div>
            <p>INSIGHTS</p>
            <h2>Latest from Qyasat</h2>
          </div>
          <span>Practical ideas about technology, automation, marketing, and AI.</span>
        </div>

        {latestPosts.length > 0 ? (
          <div className="qyasat-blog-grid">
            {latestPosts.map((post) => (
              <a className="qyasat-blog-card" href={`/${"en"}/blog/${post.slug}`} key={post.id}>
                <div className="qyasat-blog-image">
                  {post.coverImageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={post.coverImageUrl} alt={post.title.en || post.title.ar} />
                  ) : (
                    <span>QY</span>
                  )}
                </div>
                <div className="qyasat-blog-copy">
                  <small>{post.category || "Insights"}</small>
                  <h3>{post.title.en || post.title.ar}</h3>
                  <p>{post.excerpt.en || post.excerpt.ar}</p>
                  <b>Read article →</b>
                </div>
              </a>
            ))}
          ...
          <div className="qyasat-blog-empty">Published articles will appear here automatically.</div>
        )}

        <a className="qyasat-blog-all" href="/en/blog">View all articles</a>
      </section>
